...
@app.route("/")
def home_page():
    laptops = Product.query.filter_by(category_id=1).limit(4).all()
    laptop_bags = Product.query.filter_by(category_id=2).limit(4).all()
    laptop_coolers = Product.query.filter_by(category_id=3).limit(4).all()
    return render_template("main_page.html", products=Product.query.all(), categories=ProductCategory.query.all(),
                           laptop_bags=laptop_bags, User=User.query.all(), laptop_coolers = laptop_coolers, laptops=laptops )
